...
# Use us-east-1 for Bedrock (has all models available)
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info(
        ...
        extra={"requestId": request_id, "eventType": "processing_start"},
    )
    ...
    try:
        ...
        return {
            "statusCode": 200,
            "body": json.dumps(
                {"message": "Document processing started successfully", "requestId": request_id}
            ),
        }
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"requestId": request_id, "eventType": "processing_failure", "error": str(e)},
        )

        return {
            "statusCode": 500,
            "body": json.dumps(
                {"error": "Internal server error", "message": str(e), "requestId": request_id}
            ),
        }
...
def process_s3_event(record: Dict[str, Any], request_id: str) -> None:
    ...
    try:
        ...
        logger.info(f"Processing S3 object: s3://{bucket}/{key}", extra={"requestId": request_id})

        # Extract jobId from S3 key pattern: uploads/{userId}/{jobId}-{filename}
        job_id = extract_job_id_from_key(key)

        if not job_id:
            logger.error(f"Invalid S3 key format: {key}", extra={"requestId": request_id})
            raise ValueError(
                f"Invalid S3 key format. Expected: uploads/{{userId}}/{{jobId}}-{{filename}}, got: {key}"
            )

        logger.info(f"Extracted jobId: {job_id}", extra={"jobId": job_id, "requestId": request_id})
        ...
        if job_id:
            ...
            emit_processing_metric(
                ...
                unit="Count",
                job_id=job_id,
                dimensions={"Status": "Failed", "ErrorType": error_type},
            )
...
def update_job_status(
    job_id: str, status: str, message: str, metadata: Optional[Dict[str, Any]] = None
) -> None:
    """
    ...
    try:
        ...
        expression_attribute_names = {"#status": "status", "#message": "message"}
...
def extract_text_with_ocr(bucket: str, key: str, job_id: str, max_retries: int = 1) -> str:
    """
    Extract text from document (DEMO MODE - returns mock data).

    TODO: Add real OCR with AWS Textract or compatible OCR library when Python 3.11 is available.
    ...
    """
    logger.info(f"OCR extraction (DEMO MODE) for job {job_id}")
    logger.info(f"Document location: s3://{bucket}/{key}")

    # Return mock OCR text for demo
    mock_text = """Dr. Smith Medical Clinic
...
2. Ibuprofen 400mg
   - Take 1 tablet as needed for pain/fever
...
Signature: [Signed]"""
    ...
    logger.info(f"Mock OCR extraction complete: {len(mock_text)} characters")
    return mock_text


def extract_entities_with_bedrock(
    ocr_text: str, job_id: str, max_retries: int = 3
) -> Dict[str, Any]:
    """
    ...
    while attempt <= max_retries:
        try:
            attempt += 1
            logger.info(
                f"Bedrock entity extraction attempt {attempt}/{max_retries + 1} for job {job_id}"
            )
            ...
            elif "nova" in BEDROCK_MODEL_ID.lower():
                # Amazon Nova format (similar to Claude)
                request_body = {
            ...
            elif "llama" in BEDROCK_MODEL_ID.lower() or "meta" in BEDROCK_MODEL_ID.lower():
                # Meta Llama format
                request_body = {
                    "prompt": prompt,
                    "max_gen_len": 4096,
            ...
            if "anthropic" in BEDROCK_MODEL_ID.lower():
                ...
                if "output" not in response_body or "message" not in response_body["output"]:
                    raise ValueError("Invalid Nova response: missing 'output.message' field")
                ...
                if "results" in response_body and len(response_body["results"]) > 0:
                    extracted_text = response_body["results"][0].get("outputText", "")
                else:
                    raise ValueError("Invalid Titan response: missing 'results' field")
            elif "llama" in BEDROCK_MODEL_ID.lower() or "meta" in BEDROCK_MODEL_ID.lower():
                # Meta Llama response format
                if "generation" in response_body:
                    extracted_text = response_body["generation"]
                else:
                    raise ValueError("Invalid Llama response: missing 'generation' field")
            else:
                # Try Nova format as default
            ...
            logger.info(
                f"Successfully extracted entities for job {job_id}: "
...
def transform_to_fhir(structured_data: Dict[str, Any], job_id: str) -> Dict[str, Any]:
    ...
    for idx, medication in enumerate(medications):
        ...
        fhir_bundle["entry"].append(
            {"fullUrl": f"urn:uuid:medication-{job_id}-{idx}", "resource": medication_resource}
        )
    ...
    for idx, condition in enumerate(conditions):
        ...
        fhir_bundle["entry"].append(
            {"fullUrl": f"urn:uuid:condition-{job_id}-{idx}", "resource": condition_resource}
        )
    ...
    for idx, lab_result in enumerate(lab_results):
        ...
        fhir_bundle["entry"].append(
            {"fullUrl": f"urn:uuid:observation-{job_id}-{idx}", "resource": observation_resource}
        )

    logger.info(
        f"FHIR transformation complete for job {job_id}: "
...
def create_medication_statement(
    medication: Dict[str, Any], job_id: str, index: int
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "status": "active",
        "medicationCodeableConcept": {"text": medication.get("name", "Unknown medication")},
        "dosage": [
            {
                "text": f"{medication.get('dosage', 'Unknown dosage')} {medication.get('frequency', 'Unknown frequency')}",
                "timing": {"code": {"text": medication.get("frequency", "Unknown frequency")}},
                "doseAndRate": [
                    {"doseQuantity": {"value": medication.get("dosage", "Unknown"), "unit": "dose"}}
                ],
            }
        ],
        "extension": [
...
def create_observation(lab_result: Dict[str, Any], job_id: str, index: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "code": {"text": lab_result.get("testName", "Unknown test")},
        "valueQuantity": {
...
def store_results(
    ...
    try:
        ...
        error_message = e.response["Error"]["Message"]
        logger.error(
            f"DynamoDB error storing results for job {job_id}: {error_code} - {error_message}"
        )
        raise Exception(f"Failed to store results in DynamoDB: {error_message}")


def handle_processing_error(
    job_id: str, stage: str, error: Exception, processing_start_time: float
) -> None:
    """
    ...
    emit_processing_metric(
        ...
        unit="Count",
        job_id=job_id,
        dimensions={"Status": "Failed", "ErrorType": error_type, "Stage": stage},
    )
    emit_processing_metric(
        metric_name="ProcessingDuration",
        value=processing_duration,
...
def emit_processing_metric(
    ...
    try:
        metric_data = {
            ...
            "Dimensions": [{"Name": "Service", "Value": "DocumentProcessor"}],
        }

        # Add custom dimensions if provided
        if dimensions:
            for dim_name, dim_value in dimensions.items():
                metric_data["Dimensions"].append({"Name": dim_name, "Value": dim_value})

        # Put metric to CloudWatch
        cloudwatch.put_metric_data(Namespace="DocumentScanDemo", MetricData=[metric_data])
